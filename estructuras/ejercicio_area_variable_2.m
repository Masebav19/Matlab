...
for i=1:Nbarra
    msg=sprintf("El elemento %d presenta area variable?Y/N: ",i);
    if(input(msg)=='Y')
        msg=sprintf("Ingrese el valor del area inicia y el valor del area final entre corchetes separados por comas: ");
        area=input(msg);
        n_intervalos=10;
        coordi=nodo_info{barra_info(i,1),1}./n_intervalos;
        coordf=nodo_info{barra_info(i,2),1}./n_intervalos;
        Sj_tempo=zeros(6,6);
        [~,~,~,L(i)]=calculo_barra_param(nodo_info{barra_info(i,1),1},nodo_info{barra_info(i,2),1},E,area(2)-area(1));
        vector=coordf-coordi;
        for k=1:n_intervalos
          aux=zeros(6,6);
         [aux,~,~,~]=calculo_barra_param(nodo_info{barra_info(i,1),1},vector,E,((area(2)-area(1))/L(i))*(k*L(i)/n_intervalos)+area(1));
         Sj_tempo=Sj_tempo+aux;
        end
        SJ(:,:,i)=Sj_tempo;
    else
