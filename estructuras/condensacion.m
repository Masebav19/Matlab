...
disp(msg);
for i=1:Nbarra
     msg=sprintf("La Viga %d tiene carga distribuida?Y/N: ",i);
     if(input(msg)=='Y')
        msg=sprintf("Ingrese el valor de la carga: ");
        data=input(msg);
        Aj_fe(:,i)= Aj_fe(:,i)+calculo_carga_distr([1,data,0,0,6],L(i));
     end
end
...
%% Condensación 
nodo_info_conden=input("Ingrese el numero del nodo que se desplaza que el deszplazamiento entre corchetes y separados por comas [n,delta(x),delta(y)]");
nodo_conden=nodo_info_conden(1);
delta_coor=[nodo_info_conden(2),nodo_info_conden(3)];
index=zeros(Nnodos,grados_liber);
for i=1:Nnodos
    index(i,:)=[grados_liber*i-2:grados_liber*i];
end
index_c=index(nodo_conden,:);
index_b=[];
for i=1:Nnodos
   if i~=nodo_conden
      index_b=[index_b,index(i,:)]; 
   end
end

nodo_info{nodo_conden,1}=nodo_info{nodo_conden,1}+delta_coor;
...
Ac=[reacciones(index_c,1),reacciones(index_c,1),reacciones(index_c,1)];
Ab=reacciones(index_b,1);
Scc=M_rigidez_global(index_c,index_c);
Scb=M_rigidez_global(index_c,index_b);
Sbc=M_rigidez_global(index_b,index_c);
Sbb=M_rigidez_global(index_b,index_b);
